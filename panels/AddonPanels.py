...
class MBTShowPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.prop(mbt_toolpanel, "show_mhrise")
        row = layout.row()
        row.prop(mbt_toolpanel, "show_mhworld")
...
class MBTMHWorld(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(text="Batch Snap Bones / Rename Vertex Groups")
        row = layout.row()
        row.prop(mbt_toolpanel, "MHWorldBoneList")
        row.operator("mhw.open_bone_dictionary_folder", icon="FILEBROWSER")
        layout.operator("mhw.snap_bones", icon="OUTLINER_OB_ARMATURE")
        layout.operator("mhw.rename_vg", icon="OUTLINER_DATA_MESH")
...
class MBTCredits(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col = box.column(align=True)
        row = col.row(align=False)
        row.label(text="Modder Batch Tool", icon_value=preview_collections["icons"]["korone"].icon_id)
        col.separator()
        ...
        row.operator("mtb.github_website", icon_value=preview_collections["icons"]["github"].icon_id)
        row = col.row() ; row.scale_y = 1.1
        row.operator("mtb.dimcirui_github_website", icon_value=preview_collections["icons"]["github"].icon_id)
        row = col.row() ; row.scale_y = 1.1
        row.operator("mtb.bilibili_website", icon_value=preview_collections["icons"]["bilibili"].icon_id)
        row = col.row() ; row.scale_y = 1.1
        row.operator("mtb.qq_website", icon_value=preview_collections["icons"]["qq"].icon_id)
        row = col.row() ; row.scale_y = 1.1
        row.operator("mtb.caimogu_website", icon_value=preview_collections["icons"]["caimogu"].icon_id)