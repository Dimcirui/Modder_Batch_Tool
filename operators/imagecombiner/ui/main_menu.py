...
class MaterialMenu(bpy.types.Panel):
    ...
    bl_idname = 'SMC_PT_MBT_Main_Menu'
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = "Modder Batch Tool"
    bl_parent_id = "OBJECT_PT_MBT_UniversalFunction"
    ...
    def _materials_list(col: bpy.types.UILayout, scn: Scene, layout: bpy.types.UIList) -> None:
        ...
        col = layout.column()
        col.scale_y = 1.5
        col.operator('smc.combiner', text='Save Combined Image', icon="FILE_IMAGE")
    ...
    def pillow_installator(col: bpy.types.UILayout) -> None:
        ...
        row = col.row()
        row.scale_y = 1.5
        row.operator('smc.get_pillow', text='Install Pillow')
        col.separator()
        col.separator()
        col = col.box().column()
        ...
        #col.operator('smc.browser', text='shotariya#4269').link = discord
